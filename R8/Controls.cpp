...
void Slider::HandleInput()
{
	...
	if (gMenu.mb == e_mb::lclick)
		bDrag = true;
	else if (gMenu.mb == e_mb::null)
		bDrag = false;

	if (gMenu.mw != e_mw::null)
		value += (gMenu.mw == e_mw::down) ? step : -step;
	else if (bDrag && gMenu.mb == e_mb::ldown)
	{
		float percent = float(gMenu.mouse.x - x) / (float(w) - 30.f);
		if (percent > 1.f)
			percent = 1.f;
		else if (percent < 0.f)
			percent = 0.f;
...
int KeyBind::Draw(bool mouseOver)
{
	...
	if (mouseOver)
		mouseOver = !gMenu.mouseOver(x, y + TEXTH + (TEXTH / 2) - (iW / 2), iW, iW);
	SColor clr = mouseOver ? HTEXT : CTEXT;
	...
	gDraw.DrawRect(iX, y + TEXTH, w - iW - 5, TEXTH, SColor(42, 42, 48));
	gDraw.OutlineRect(iX, y + TEXTH, w - iW - 5, TEXTH, SColor(60));

	string keyStr = bEnabled ? mode == e_kbmode::always ? "Always on" : gKey.KeyToString(key) : "Off";
	gDraw.DrawString(iX + 3, y + TEXTH, mouseOver ? clr : SColor(125, 125, 130), keyStr, gFonts.verdana_bold);

	gDraw.DrawRect(iX + w - iW - 15, y + TEXTH + 5, 6, 1, SColor(125, 125, 140));
	gDraw.DrawRect(iX + w - iW - 15, y + TEXTH + 7, 6, 1, SColor(125, 125, 140));
	gDraw.DrawRect(iX + w - iW - 15, y + TEXTH + 9, 6, 1, SColor(125, 125, 140));
...
void KeyBind::HandleInput()
{
	...
	if (gMenu.mouseOver(x, y + TEXTH + (TEXTH / 2) - (iW / 2), iW, iW))
		return;
